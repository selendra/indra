...
pub struct RFullNode<C> {
	...
	pub relay_chain_full_node: selendra_service::NewFull<C>,
	/// The collator key used by the node.
	pub collator_key: CollatorPair,
}

impl<C> Deref for RFullNode<C> {
	type Target = selendra_service::NewFull<C>;
	...
	IQ: ImportQueue<Block> + 'static,
{
	...
	relay_chain_full_node.client.execute_with(StartPoVRecovery {
		...
		overseer_handle: relay_chain_full_node
			.overseer_handle
			.clone()
			.ok_or_else(|| "Selendra full node did not provide an `OverseerHandle`!")?,
		_phantom: PhantomData,
	})?;
...
impl<'a, Block, Client, Backend> selendra_service::ExecuteWithClient
...
impl<'a, Block, Client, IQ> selendra_service::ExecuteWithClient
...
pub fn build_selendra_full_node(
	config: Configuration,
	telemetry_worker_handle: Option<TelemetryWorkerHandle>,
) -> Result<RFullNode<PClient>, selendra_service::Error> {
	...
	if is_light {
		...
		let relay_chain_full_node = selendra_service::build_full(
			config,
			selendra_service::IsCollator::Yes(collator_key.clone()),
			None,
			true,
			None,
