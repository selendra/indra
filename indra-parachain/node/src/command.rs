...
use indra_runtime::{Block, RuntimeApi};
use log::info;
use sc_cli::{
...
fn load_spec(id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
	Ok(match id {
		...
		"indra" => Box::new(chain_spec::indra_config()),
		"indra-testnet" => Box::new(chain_spec::indra_tesnet_config()?),
...
impl SubstrateCli for Cli {
	...
	fn description() -> String {
		...
		indra <parachain-args> -- <relay-chain-args>"
			.into()
	}
	...
	fn support_url() -> String {
		"https://github.com/selendra/indra/issues/new".into()
	}
	...
	match &cli.subcommand {
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				crate::service::start_parachain_node(config, selendra_config, id)
					.await
					.map(|r| r.0)
					.map_err(Into::into)
			})
