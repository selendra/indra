...
impl sc_executor::NativeExecutionDispatch for RuntimeExecutor {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		indra_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		indra_runtime::native_version()
	}
}
	...
	>,
{
	...
	let params = PartialComponents {
		...
		transaction_pool,
		select_chain: (),
		other: (telemetry, telemetry_worker_handle, frontier_backend, fee_history_cache),
	};
...
where
	...
	BIC: FnOnce(
		...
		Option<TelemetryHandle>,
		&TaskManager,
		&selendra_service::NewFull<selendra_service::Client>,
		Arc<
			sc_transaction_pool::FullPool<
				Block,
				TFullClient<Block, RuntimeApi, NativeElseWasmExecutor<Executor>>,
	...
	) -> Result<Box<dyn ParachainConsensus<Block>>, sc_service::Error>,
{
	...
	let (mut telemetry, telemetry_worker_handle, frontier_backend, fee_history_cache) =
		params.other;

	let relay_chain_full_node =
		cumulus_client_service::build_selendra_full_node(selendra_config, telemetry_worker_handle)
			.map_err(|e| match e {
	...
	let rpc_extensions_builder = {
		...
		Box::new(move |deny_unsafe, subscription| {
			...
			Ok(rpc::create_full(deps, subscription, overrides.clone()))
		})
	};
...
pub async fn start_parachain_node(
	...
	TaskManager,
	Arc<TFullClient<Block, indra_runtime::RuntimeApi, NativeElseWasmExecutor<RuntimeExecutor>>>,
)> {
	start_node_impl::<indra_runtime::RuntimeApi, RuntimeExecutor, _, _>(
        parachain_config,
        selendra_config,
        id,
        |client,
