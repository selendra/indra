...
fn collect_relay_storage_proof(
	selendra_backend: &impl Backend<PBlock>,
	para_id: ParaId,
	relay_parent: PHash,
) -> Option<sp_state_machine::StorageProof> {
	use relay_chain::well_known_keys as relay_well_known_keys;

	let relay_parent_state_backend = selendra_backend
...
impl ParachainInherentData {
		...
		PClient::Api: ParachainHost<PBlock>,
	{
		let relay_chain_state =
			collect_relay_storage_proof(selendra_backend, para_id, relay_parent)?;
		let downward_messages = retrieve_dmq_contents(selendra_client, para_id, relay_parent)?;
		let horizontal_messages =
			retrieve_all_inbound_hrmp_channel_contents(selendra_client, para_id, relay_parent)?;
	...
	pub fn create_at_with_client(
		relay_parent: PHash,
		selendra_client: &Client,
		relay_chain_backend: &impl Backend<PBlock>,
		validation_data: &PersistedValidationData,
		para_id: ParaId,
	) -> Option<ParachainInherentData> {
		selendra_client.execute_with(CreateAtWithClient {
			relay_chain_backend,
			validation_data,
			para_id,
			relay_parent,
