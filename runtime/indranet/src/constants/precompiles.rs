...
/// The PrecompileSet installed in the indranet runtime.
#[derive(Debug, Clone, Copy)]
pub struct IndranetPrecompiles<R>(PhantomData<R>);

impl<R> IndranetPrecompiles<R> {
	#[allow(dead_code)]
	pub fn new() -> Self {
...
impl<R> PrecompileSet for IndranetPrecompiles<R>
where
	R: pallet_evm::Config,
	...
	R::Call: Dispatchable<PostInfo = PostDispatchInfo> + GetDispatchInfo + Decode,
{
	fn execute(
		...
		match address {
			...
			// indranet precompiles (starts from 0x5000):
			// Default
			_ => None,
		}
	}
