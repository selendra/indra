	...
	F: Formatter,
{
			...
			.map_err(|err| internal_err(format!("{:?}", err)))?
			{
				...
				None => {
					...
					let ethereum_transactions: Vec<EthereumTransaction> = if api_version > 1 {
						...
						#[allow(deprecated)]
						let legacy = api.extrinsic_filter_before_version_2(&best_block, xts).map_err(
							|err| {
								internal_err(format!(
									"fetch runtime extrinsic filter failed: {:?}",
									err
						...
						if !is_eip1559 {
							...
							match receipt {
								...
								ethereum::ReceiptV3::EIP1559(d) => {
									...
									(d.logs, d.logs_bloom, d.status_code, cumulative_gas, gas_used)
								},
							}
						};
	...
	fn fee_history(
		...
		if let Ok(Some(id)) = frontier_backend_client::native_block_id::<B, C>(
			...
			let header = if let Ok(Some(header)) = self.client.header(id) {
				header
			} else {
				return Err(internal_err(format!("Block {} don't exist", id)))
			};
